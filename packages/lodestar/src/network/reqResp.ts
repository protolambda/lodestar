  ...
  public sendResponseStream(id: RequestId, err: RpcError|null, chunkIter: AsyncIterable<ResponseBody>): void {
    if(err) {
      ...
      this.responseListener.emit(createResponseEvent(id), async function* () {
        yield {status: err.status, requestId: id, body: encodeP2pErrorMessage(config, err.message || "")};
      }());
      ...
      return (async function * () {
        for await (const request of source) {
          if (!request.isValid) {
            ...
            yield* getResponse(peerId, method, encoding, request.requestId, request.body);
          }
          return;
        }
        yield* getResponse(peerId, method, encoding, randomRequestId());
      })();
    };
  }
